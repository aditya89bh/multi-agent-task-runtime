...
class RuntimeCoordinator:
    ...
    def execute_agent(self, agent: BaseAgent, **context: Any) -> Any:
        ...
        self.event_bus.publish(Event(event_type=AGENT_STARTED, agent_id=agent.agent_id, payload={"context": context}))
        agent.started = True
        result = agent.run(**context)
        self.execution_context["results"][agent.agent_id] = result
        agent.finished = True
        self.event_bus.publish(Event(event_type=AGENT_FINISHED, agent_id=agent.agent_id, payload={"result": result}))
        return result
